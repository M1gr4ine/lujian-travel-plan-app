...
- Emit the complete mobile headline in the source HTML under exactly one `[data-lujian-cover]` node. Do not rely on JavaScript to populate it: the Android thumbnail renderer disables JavaScript and otherwise falls back to the plan title.
...
- Disable JavaScript and confirm the single `[data-lujian-cover]` node still contains the complete shared `headline`; a `640px` Android thumbnail capture must show that headline instead of the plan title.
