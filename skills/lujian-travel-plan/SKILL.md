...
- Keep `title` as the concise Android library/page brand name, defaulting to `<destination>旅行计划`; place day/night counts in `coverSubtitle`, never in the plan title.
...
- Emit exactly one static `[data-lujian-cover]` block containing non-empty `.brand-title`, `.brand-sub`, and the shared `h1`. `.brand-title` must equal the embedded `title`; use reviewed `trip.coverSubtitle` for `.brand-sub`, otherwise derive only from known trip metadata. The Android library renders this text natively into the plan-name bar, divider, large headline, gold wave, and corner dot; never leave the block to JavaScript or fall back to the generic plan title.
...
- Confirm the single `[data-lujian-cover]` block exposes `.brand-title` equal to the embedded `title`, plus the reviewed `.brand-sub` and complete shared `h1`, all without JavaScript. Reimport/reindex in Android and verify the `640px` native thumbnail shows the plan-name bar, divider, large headline, gold wave, and corner dot instead of `fallbackThumbnail`.
